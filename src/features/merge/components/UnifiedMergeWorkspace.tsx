...
import { Download, Loader2, ArrowRight, FileImage, PlusCircle, FileText, Trash2, GripVertical } from 'lucide-react'
import type { MergeSettings, MergeNamingSettings } from './MergeProcessor'
...
export interface UnifiedMergeWorkspaceProps {
  ...
  namingSettings: MergeNamingSettings
  setNamingSettings: Dispatch<SetStateAction<MergeNamingSettings>>
...
export function UnifiedMergeWorkspace({
  ...
  const allImages = useMemo(() => files.length > 0 && files.every(f => f.type.startsWith('image/')), [files])
    ...
    <div className="flex-1 flex flex-col h-full bg-[#f8f9fa] dark:bg-[#111] overflow-hidden relative">
      ...
      <div className="shrink-0 bg-white/80 dark:bg-[#1a1a1a]/80 backdrop-blur-md border-b border-[var(--border-secondary)] px-6 py-2.5 grid grid-cols-3 items-center">
        ...
        <div className="flex justify-center items-center gap-2 text-sm font-semibold">
          ...
          <span className={`px-3 py-1 rounded-full whitespace-nowrap ${step >= 2 ? 'bg-[var(--color-primary-500)] text-white' : 'bg-gray-200 text-gray-500'}`}>2. Arrange & Settings</span>
          <ArrowRight size={14} className="text-gray-400 shrink-0" />
          <span className={`px-3 py-1 rounded-full whitespace-nowrap ${step >= 3 ? 'bg-[var(--color-primary-500)] text-white' : 'bg-gray-200 text-gray-500'}`}>3. Review</span>
        </div>
      ...
      <div className={`mx-auto w-full flex flex-col flex-1 min-h-0 ${step === 2 ? 'p-4 md:p-6 max-w-[1920px]' : 'p-6 md:p-8 max-w-5xl overflow-y-auto custom-scrollbar'}`}>
        
        {/* Step 1: Upload Documents */}
        {step === 1 && (
          <div className="animate-in fade-in zoom-in-95 duration-500">
            <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-2 text-center">Merge Documents</h1>
            <p className="text-center text-[var(--text-secondary)] mb-10">Combine multiple Images and PDFs into a single document</p>
            ...
            {files.length > 0 && (
              <div className="mt-8 flex justify-center">
                 <Button variant="primary" size="lg" onClick={onProceedToReview}>
                   Continue to Arrange ({files.length} files) <ArrowRight className="ml-2" />
                 </Button>
              </div>
            )}
          </div>
        ...
        {step === 2 && (
          <div className="flex-1 grid grid-cols-[260px_1fr_260px] xl:grid-cols-[300px_1fr_300px] gap-4 md:gap-6 animate-in fade-in duration-500 min-h-0">
            ...
            <div className="flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-2 pb-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select 
                      ...
                      onChange={(e) => setNamingSettings((s: MergeNamingSettings) => ({ ...s, relationship: e.target.value }))}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-primary)] mb-1">Document Base Name</label>
                    <input 
                      type="text"
                      placeholder="e.g. AllDocuments"
                      value={namingSettings.documentType || ''}
                      onChange={(e) => setNamingSettings((s: MergeNamingSettings) => ({ ...s, documentType: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setNamingSettings((s: MergeNamingSettings) => ({ ...s, suffix: e.target.value }))}
                      className="w-full p-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-transparent text-sm"
                    />
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-[var(--border-secondary)] overflow-hidden p-4">
                <h3 className="font-bold text-xs uppercase tracking-wider text-[var(--text-secondary)] mb-3">Page Layout</h3>
                <div className="space-y-3">
                  {allImages && (
                    <div>
                      <label className="block text-xs font-semibold text-[var(--text-primary)] mb-1">Output Format</label>
                      <select 
                        value={settings.outputFormat}
                        onChange={(e) => setSettings(s => ({ ...s, outputFormat: e.target.value as 'pdf' | 'image' }))}
                        className="w-full p-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-transparent font-medium text-sm"
                      >
                  ...
                  {(!allImages || settings.outputFormat === 'pdf') ? (
                    <>
                      <div>
                        <label className="block text-xs font-semibold text-[var(--text-primary)] mb-1">Page Size</label>
                        <select 
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-[var(--text-primary)] mb-1">Orientation</label>
                        <select 
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[var(--text-primary)] mb-1">Image Layout</label>
                      <select 
                        value={settings.imageLayout}
                        onChange={(e) => setSettings(s => ({ ...s, imageLayout: e.target.value as 'vertical' | 'horizontal' }))}
                        className="w-full p-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-transparent font-medium text-sm"
                      >
            ...
            <div className="flex flex-col bg-white dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-[var(--border-secondary)] overflow-hidden min-w-[500px]">
              <div className="p-4 border-b border-[var(--border-secondary)] bg-gray-50 dark:bg-[#151515] flex justify-between items-center">
                <h3 className="font-bold text-sm text-[var(--text-primary)]">Arrange Files</h3>
                <span className="text-xs text-[var(--text-secondary)]">Drag items to reorder</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50/50 dark:bg-[#151515]/50">
                {files.map((file, index) => {
                  ...
                  return (
                    ...
                    >
                      <GripVertical size={20} className="text-gray-400 group-hover:text-gray-600" />
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[var(--text-secondary)]">
                          {isPdf ? 'PDF Document' : 'Image'} • {(file.size / 1024).toFixed(1)} KB
                        </p>
                      </div>
                ...
                <div className="mt-4 pt-4 border-t border-dashed">
                  <label className="flex items-center justify-center w-full p-4 border-2 border-dashed border-blue-200 rounded-xl bg-blue-50/50 hover:bg-blue-50 cursor-pointer transition-colors text-blue-600 font-semibold text-sm">
                    ...
                    <input type="file" multiple className="hidden" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(e) => {
            ...
            <div className="flex flex-col overflow-y-auto custom-scrollbar pb-6 pr-2">
              <div className="bg-white dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-[var(--border-secondary)] p-4 flex flex-col flex-1 relative overflow-hidden">
                <h3 className="font-bold text-xs uppercase tracking-wider text-[var(--text-secondary)] mb-3">Merge Summary</h3>
                ...
                {isProcessing && (
                  <div className="absolute inset-0 bg-white/50 dark:bg-black/50 backdrop-blur-[1px] flex items-center justify-center z-10">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="font-bold text-sm text-[var(--color-primary-600)]">Processing...</span>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-3 text-xs mb-4">
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-2.5 rounded-lg border border-blue-100 dark:border-blue-800/30">
                    <p className="text-[10px] text-blue-600 dark:text-blue-400 font-bold mb-1 uppercase tracking-wide">Target Filename</p>
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-800">
                    ...
                    <span className="font-bold text-gray-800 dark:text-gray-200 text-sm">{files.length}</span>
                  </div>
                  
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-800">
                    <span className="text-[var(--text-secondary)]">Total Inputs Size</span>
                    <span className="font-bold text-[var(--color-primary-600)]">{(totalSize / (1024 * 1024)).toFixed(2)} MB</span>
                  </div>
                </div>
                ...
                <div className="w-48 h-48 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white flex items-center justify-center p-2 shadow-sm shrink-0">
                  ...
                  {!processedBlobUrl && (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs font-bold text-gray-500 text-center">PDF<br/>DOCUMENT</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 space-y-3 w-full">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm pt-2">
                    <div className="flex flex-col">
                      <span className="text-[var(--text-secondary)] text-xs">Page Size</span>
                      <span className="font-semibold text-[var(--text-primary)]">{settings.pageSize}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[var(--text-secondary)] text-xs">Files Merged</span>
                      <span className="font-semibold text-[var(--text-primary)]">{files.length}</span>
                    </div>
                  </div>
                </div>
              </div>
