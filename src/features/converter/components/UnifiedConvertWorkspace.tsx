...
export function UnifiedConvertWorkspace({
  ...
  const processedBlobUrl = useMemo(() => isImageBlob ? URL.createObjectURL(processedBlob) : null, [processedBlob, isImageBlob])
    ...
    <div className="flex-1 flex flex-col h-full bg-[#f8f9fa] dark:bg-[#111] overflow-hidden relative">
      ...
      <div className="shrink-0 bg-white/80 dark:bg-[#1a1a1a]/80 backdrop-blur-md border-b border-[var(--border-secondary)] px-6 py-2.5 grid grid-cols-3 items-center">
        ...
        <div className="flex justify-center items-center gap-2 text-sm font-semibold">
          ...
          <span className={`px-3 py-1 rounded-full whitespace-nowrap ${step >= 2 ? 'bg-[var(--color-primary-500)] text-white' : 'bg-gray-200 text-gray-500'}`}>2. Settings</span>
          <ArrowRight size={14} className="text-gray-400 shrink-0" />
      ...
      <div className={`mx-auto w-full flex flex-col flex-1 min-h-0 ${step === 2 ? 'p-4 md:p-6 max-w-7xl' : 'p-6 md:p-8 max-w-5xl overflow-y-auto custom-scrollbar'}`}>
        
        {step === 1 && (
          <div className="animate-in fade-in zoom-in-95 duration-500">
            <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-2 text-center">Format Converter</h1>
            <p className="text-center text-[var(--text-secondary)] mb-10">Convert images to PDF, or PDF to images</p>
            ...
            {files.length > 0 && (
              <div className="mt-8 flex justify-center">
                 <Button variant="primary" size="lg" onClick={onProceedToReview}>
                   Continue to Settings ({files.length} files) <ArrowRight className="ml-2" />
                 </Button>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 grid grid-cols-[260px_1fr_260px] xl:grid-cols-[300px_1fr_300px] gap-4 md:gap-6 animate-in fade-in duration-500 min-h-0">
            ...
            <div className="flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-2 pb-6">
              <div className="bg-white dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-[var(--border-secondary)] overflow-hidden p-4">
                <h3 className="font-bold text-xs uppercase tracking-wider text-[var(--text-secondary)] mb-3">Conversion Settings</h3>
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-primary)] mb-1">Target Format</label>
                    <select 
            ...
            <div className="flex flex-col bg-white dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-[var(--border-secondary)] overflow-hidden min-w-[400px]">
              <div className="p-4 border-b border-[var(--border-secondary)] bg-gray-50 dark:bg-[#151515] flex justify-between items-center">
                <h3 className="font-bold text-sm text-[var(--text-primary)]">Files to Convert</h3>
            ...
            <div className="flex flex-col overflow-y-auto custom-scrollbar pb-6 pr-2">
              <div className="bg-white dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-[var(--border-secondary)] p-4 flex flex-col flex-1 relative overflow-hidden">
                <h3 className="font-bold text-xs uppercase tracking-wider text-[var(--text-secondary)] mb-3">Conversion Summary</h3>
                ...
                <div className="space-y-3 text-xs mb-4">
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-2.5 rounded-lg border border-blue-100 dark:border-blue-800/30">
                    <p className="text-[10px] text-blue-600 dark:text-blue-400 font-bold mb-1 uppercase tracking-wide">Target Format</p>
                    <p className="font-mono text-gray-800 dark:text-gray-200 break-all leading-tight uppercase">{settings.targetFormat}</p>
                  </div>
                ...
                <div className="w-48 h-48 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white flex items-center justify-center p-2 shadow-sm shrink-0">
                  ...
                  {!processedBlobUrl && (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs font-bold text-gray-500 text-center uppercase">{settings.targetFormat === 'pdf' ? 'PDF DOCUMENT' : (processedBlob.type === 'application/zip' ? 'ZIP ARCHIVE' : 'DOCUMENT')}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 space-y-3 w-full">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm pt-2">
                    <div className="flex flex-col">
                      <span className="text-[var(--text-secondary)] text-xs">Target Format</span>
                      <span className="font-semibold text-[var(--text-primary)] uppercase">{settings.targetFormat}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[var(--text-secondary)] text-xs">Files Converted</span>
